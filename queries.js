    ...
    if (err) {
      ...
      if (check != -1) {
        pool.query(
          {
            text:
              'SELECT * FROM "Chapter" WHERE manga_id = $1 AND chap_name = $2',
              ...
              if (result.rowCount != 0) {
                ...
                pool.query(
                  ...
                  (error, result) => {
                    ...
                    response
                      .status(201)
                      .send(`Added chapter to manga with id: ${manga_id}`);
                  }
                );
              }
            }
...
const updateChapter = (request, response) => {
  ...
  const { chap_name, chap_content, time } = request.body;
  if (chap_name == null) {
...
const getComment = (request, response) => {
  ...
  pool.query(
    {
      text:
        'SELECT user_name,content,time_up FROM "Comment" WHERE manga_id = $1 AND chap_id = $2',
      values: [mangaId, chapId]
    },
    (error, result) => {
      if (error) {
...
const addComment = (request, response) => {
  const mangaId = parseInt(request.params.mangaid);
  const chapId = parseInt(request.params.chapterid);
  const { content, time } = request.body;
  if (content == null) {
    response.sendStatus(400);
  } else {
    //verify user
    jwt.verify(request.token, secret, (err, authData) => {
      if (err) {
        ...
        pool.query(
          {
            text:
              'INSERT INTO "Comment"(user_name,manga_id,chap_id,content) VALUES($1,$2,$3,$4)',
        ...
        (error, result) => {
          if (result.rowCount == 0) {
            pool.query(
              {
                text:
                  'INSERT INTO "Rating" (manga_id,user_name,point) VALUES($1,$2,$3)',
            ...
            pool.query(
              {
                text:
                  'UPDATE "Rating" SET point = $3 WHERE manga_id = $1 AND user_name = $2',
                values: [mangaId, username, point]
              },
              (error, result) => {
                if (error) {
